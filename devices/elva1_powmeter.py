...
class DeviceConnection(TelnetConnection):
    ...
    def read_all(self):
        """Read power from meter, return list with value in W (or mW if meter responds in µW)"""
        try:
        ...
        try:
            ...
            if unit == 'UW':
                value = float(p[0]) / 1000.0   # µW → mW
            elif unit == 'MW':
                value = float(p[0])             # already mW
            elif unit == 'DBM':
                # unit:pow w was not accepted; reconnect will re-send it
                logging.warning(f"ELVA-1 responded in dBm on {self.host} — unit setting lost, reconnecting")
